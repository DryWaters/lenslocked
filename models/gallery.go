...
import (
	"database/sql"
	"fmt"
	"github.com/drywaters/lenslocked/errors"
	"io"
	"io/fs"
	"os"
	"path/filepath"
...
func (service *GalleryService) ById(id int) (*Gallery, error) {
	...
	row := service.DB.QueryRow(`
		SELECT title, user_id FROM
		galleries WHERE id = $1;`, gallery.ID)
...
func (service *GalleryService) ByUserId(userId int) ([]Gallery, error) {
	rows, err := service.DB.Query(`
		...
		galleries WHERE user_id = $1;`, userId)
	if err != nil {
...
func (service *GalleryService) CreateImage(galleryID int, filename string, contents io.ReadSeeker) error {
	...
	if err != nil {
		return fmt.Errorf("creating gallery %d iamges directory: %w", galleryDir, err)
	}
